// ============================================
// facilityDetails.service.js
// ============================================

const statusCode = require("../enums/statusCode");
const CustomError = require("../exceptions/CustomError");
const FacilityDetails = require("../models/FacilityDetails");
...
const add = async (req, res) => {
  ...
  try {
    ...
    const categoryExists = await FacilityCategories.findById(data.facility_category_id);
    if (!categoryExists) {
      if (file) {
        const tempPath = path.join(UPLOADS_ROOT, "facilityDetails", file.filename);
    ...
    // Handle image upload
    if (file) {
      const imageRelativePath = path.join("uploads", "facilityDetails", file.filename);
      const imageFullPath = path.join(UPLOADS_ROOT, "facilityDetails", file.filename);
      uploadedFile = imageFullPath;
      data.image_url = imageRelativePath;
    }

    return await FacilityDetails.create(data);
  } catch (error) {
...
const update = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(body.id)) {
    throw new CustomError("Invalid facility detail ID format", statusCode.BAD_REQUEST);
  }

  const detail = await FacilityDetails.findById(body.id);
  if (!detail) {
    if (file) {
      const tempPath = path.join(UPLOADS_ROOT, "facilityDetails", file.filename);
      if (fs.existsSync(tempPath)) fs.unlinkSync(tempPath);
    }
    throw new CustomError("Facility detail not found", statusCode.NOT_FOUND);
  }

  try {
    // If category is being changed, verify new category exists
    if (body.facility_category_id && body.facility_category_id !== detail.facility_category_id.toString()) {
      const categoryExists = await FacilityCategories.findById(body.facility_category_id);
      if (!categoryExists) {
        if (file) {
          const tempPath = path.join(UPLOADS_ROOT, "facilityDetails", file.filename);
          if (fs.existsSync(tempPath)) fs.unlinkSync(tempPath);
    ...
    // Handle new image
    if (file) {
      const newImageRelPath = path.join("uploads", "facilityDetails", file.filename);
      const newImageFullPath = path.join(UPLOADS_ROOT, "facilityDetails", file.filename);
...
const remove = async (id) => {
  ...
  const detail = await FacilityDetails.findById(id);
  if (!detail) {
    throw new CustomError("Facility detail not found", statusCode.NOT_FOUND);
  }
  ...
  return await FacilityDetails.findByIdAndDelete(id);
};
...
const getById = async (id) => {
  try {
    ...
    const detail = await FacilityDetails.findById(id).populate({
      path: "facility_category_id",
...
const getAll = async () => {
  try {
    const details = await FacilityDetails.find()
      .populate({
        path: "facility_category_id",
        populate: { path: "facility_id" },
      })
...
const getDetailsByCategoryId = async (categoryId) => {
  try {
    ...
    const details = await FacilityDetails.find({ facility_category_id: categoryId })
      .populate({
        path: "facility_category_id",
        populate: { path: "facility_id" },
      })

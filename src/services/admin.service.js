...
const add = async (data) => {
  try {
    ...
    // EMAIL SENDING DISABLED - Just log the credentials
...
const requestOTP = async (email) => {
  try {
    ...
    // Use hardcoded OTP instead of generating random one
    const otp = HARDCODED_OTP;
    otpStore[email] = { otp, expiresAt: Date.now() + 5 * 60 * 1000 }; // 5 minutes expiry

    // EMAIL SENDING DISABLED - Just log the OTP
    ...
    return `OTP has been sent successfully to the email: ${email}. [DEV MODE: Check console for OTP]`;
  } catch (error) {
...
// Helper function - DISABLED (using hardcoded OTP instead)
// function generateOTP() {
//   return Math.floor(100000 + Math.random() * 900000);
// }

// NODEMAILER CONFIGURATION - DISABLED
...
// EMAIL SENDING FUNCTION - DISABLED
...
//  const subject = 'Welcome to R.B. Science Admin Panel';
//     const text = `
// Hello ${data.name || ''},

// Your admin account has been created for R.B. Science Admin Panel.
...
//     otpStore[email] = { otp, expiresAt: Date.now() + 5 * 60 * 1000 }; // 5 minutes expiry
// var subject='Password Reset OTP for R.B. Science Admin Panel';
//  var text= `
// Hello,

// We received a request to reset the password for your R.B. Science Admin Panel account.
